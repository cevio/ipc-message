...
module.exports = class IPCMessager {
  constructor(isAgent) {
  ...
  send(to, url, body) {
    ...
    if (this.type === 'master') {
      return this[sending](this[collectWorkers](to, this.pid), {
        url, body
      });
    }
    process.send({
  ...
  [onMessage](msg) {
    if (this.type === 'master') {
      ...
      if (pools.indexOf(process) > -1) {
        ...
        if (this.onMessageReceive) this.onMessageReceive(msg);
      }
      if (msg.transfer && pools.length) {
        msg.to = pools.map(p => (p.process || p).pid);
        this[sending](pools, msg);
      }
    } else {
      if (msg.to.indexOf(this.pid) > -1 && this.onMessageReceive) this.onMessageReceive(msg);
    }
  }
